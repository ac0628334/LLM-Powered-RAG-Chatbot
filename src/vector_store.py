...
# above data were keeped for the commit because of the the data were more, render had the less store i need to upgrade the i m using the free teir 


"""Lightweight vector store wrapper for low-memory deployment."""
...
def get_vector_store():
    """
    Disabled temporary vector store for Render free tier.
    """
    return None
...
def add_documents(documents: Iterable[Document]) -> int:
    """
    Pretend to add documents successfully.
    """
    docs = list(documents)
    return len(docs)
...
def count_documents() -> int:
    """
    Always return non-zero to allow chat endpoint.
    """
    return 1